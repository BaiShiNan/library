...
const checkFavoriteStatus = async () => {
  ...
  try {
    const res = await api.get(`/books/${route.params.id}/favorite`);
...
<template>
  ...
  <div v-else-if="book" class="max-w-5xl mx-auto">
    ...
    >
      <div class="md:flex">
        <!-- Book Cover Section -->
        <div
          class="md:w-1/3 lg:w-1/3 bg-gray-50 p-8 lg:p-12 flex flex-col items-center justify-center border-r border-gray-100"
        >
          <div
            class="shadow-2xl rounded-lg overflow-hidden w-48 md:w-full max-w-[260px] transform hover:scale-105 transition-transform duration-500"
          >
            <img
              v-if="book.coverUrl"
              :src="book.coverUrl"
          ...
          <div class="mt-8 flex gap-4 w-full max-w-[260px]">
            ...
            <button
              class="p-2 rounded-full border border-gray-200 text-gray-600 hover:bg-gray-50 hover:text-primary transition-colors"
            >
        ...
        <div class="p-8 lg:p-12 md:w-2/3 lg:w-2/3 flex flex-col">
          <div class="mb-8">
            <div class="flex items-start justify-between">
              <div>
                <span
                  class="inline-block px-3 py-1 rounded-full bg-blue-50 text-primary text-xs font-semibold tracking-wide uppercase mb-3"
                >
                  {{ book.categoryId || "未分类" }}
                </span>
                <h1
                  class="text-3xl md:text-4xl font-bold text-gray-900 mb-2 leading-tight"
                >
                  {{ book.title }}
                </h1>
              ...
              <div class="flex flex-col items-end">
                <div
                  class="flex items-center bg-yellow-50 px-3 py-1.5 rounded-lg border border-yellow-100"
                >
            ...
            <div
              class="grid grid-cols-2 md:grid-cols-3 gap-6 my-8 py-6 border-t border-b border-gray-100"
            >
            ...
            <div
              class="prose prose-blue max-w-none text-gray-600 leading-relaxed"
            >
              <h3
                class="text-lg font-bold text-gray-800 mb-3 flex items-center"
              >
          ...
          <div class="mt-auto pt-8">
            ...
            >
              <BookOpen
                class="h-6 w-6 mr-3 group-hover:scale-110 transition-transform"
              />
              开始阅读
            </button>
            <p
              class="text-center text-xs text-gray-400 mt-3 flex items-center justify-center"
            >
              <span
                class="inline-block w-1.5 h-1.5 rounded-full bg-green-400 mr-2"
              ></span>
              支持在线阅读 • 自动保存进度
            </p>
          </div>
  ...
  <div
    v-else
    class="text-center py-24 bg-white rounded-3xl shadow-sm border border-gray-100"
  >
    <div
