  ...
  <div class="container mx-auto p-4">
    ...
    <div class="flex border-b mb-4">
      <button 
        ...
        :class="activeTab === 'books' ? 'border-b-2 border-primary font-bold text-primary' : 'text-gray-600 hover:text-primary'"
      ...
      <button 
        class="py-2 px-4 focus:outline-none transition-colors" 
        :class="activeTab === 'users' ? 'border-b-2 border-primary font-bold text-primary' : 'text-gray-600 hover:text-primary'"
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
        <div class="relative w-full md:w-96">
            <input 
                ...
                type="text" 
                :placeholder="activeTab === 'books' ? '搜索书名、作者...' : '搜索用户名、邮箱...'" 
          ...
          <div class="p-6">
            <form @submit.prevent="uploadBook" class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div class="col-span-1">
                ...
                <input v-model="newBook.author" type="text" required class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
              </div>
              <div class="col-span-1">
                <label class="block text-sm font-medium text-gray-700 mb-1">ISBN</label>
                <input v-model="newBook.isbn" type="text" required class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
              </div>
              <div class="col-span-1">
                <label class="block text-sm font-medium text-gray-700 mb-1">分类ID</label>
                <input v-model="newBook.categoryId" type="number" required class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" />
              </div>
              <div class="col-span-1 md:col-span-2">
                <label class="block text-sm font-medium text-gray-700 mb-1">描述</label>
                <textarea v-model="newBook.description" required class="w-full border rounded-lg p-2 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none h-24"></textarea>
              </div>
              <div class="col-span-1">
          ...
          <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
              <th class="py-3 px-4 w-12 text-center">
                  <input type="checkbox" @change="toggleAllBooks" :checked="isAllBooksSelected" class="rounded text-primary focus:ring-primary" />
              </th>
              <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-16">ID</th>
              <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/3">标题</th>
              <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/4">作者</th>
              <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">ISBN</th>
              <th class="py-3 px-4 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider w-24">操作</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="book in books" :key="book.id" class="hover:bg-gray-50 transition-colors">
              <td class="py-3 px-4 text-center">
                  <input type="checkbox" :value="book.id" v-model="selectedBooks" class="rounded text-primary focus:ring-primary" />
              </td>
            ...
            <tr v-if="books.length === 0">
                <td colspan="6" class="py-8 text-center text-gray-500">暂无图书数据</td>
            </tr>
          </tbody>
        </table>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table class="min-w-full">
          <thead class="bg-gray-50 border-b border-gray-100">
            <tr>
              <th class="py-3 px-4 w-12 text-center">
                  <input type="checkbox" @change="toggleAllUsers" :checked="isAllUsersSelected" class="rounded text-primary focus:ring-primary" />
              </th>
              <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-16">ID</th>
              <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/3">邮箱</th>
              <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider w-1/4">用户名</th>
              <th class="py-3 px-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">角色</th>
              <th class="py-3 px-4 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider w-24">操作</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="user in users" :key="user.id" class="hover:bg-gray-50 transition-colors">
              <td class="py-3 px-4 text-center">
                  <input type="checkbox" :value="user.id" v-model="selectedUsers" :disabled="user.role === 'ADMIN'" class="rounded text-primary focus:ring-primary" />
              </td>
              <td class="py-3 px-4 text-gray-500">{{ user.id }}</td>
              <td class="py-3 px-4 font-medium text-gray-900">{{ user.email }}</td>
              <td class="py-3 px-4 text-gray-600">{{ user.name }}</td>
              <td class="py-3 px-4">
                  <span :class="user.role === 'ADMIN' ? 'bg-purple-100 text-purple-700' : 'bg-gray-100 text-gray-600'" class="px-2 py-1 rounded-full text-xs font-medium">
                      {{ user.role }}
                  </span>
              </td>
              <td class="py-3 px-4 text-center">
             ...
             <tr v-if="users.length === 0">
                <td colspan="6" class="py-8 text-center text-gray-500">暂无用户数据</td>
            </tr>
          </tbody>
        </table>
      </div>
...
const toggleAllUsers = (e: Event) => {
    const checked = (e.target as HTMLInputElement).checked;
    if (checked) {
        selectedUsers.value = users.value.filter(u => u.role !== 'ADMIN').map(u => u.id);
    } else {
        selectedUsers.value = [];
    }
};
...
const batchDeleteBooks = async () => {
    if (!confirm(`确定要删除选中的 ${selectedBooks.value.length} 本书吗？`)) return;
    try {
...
const batchDeleteUsers = async () => {
    if (!confirm(`确定要注销选中的 ${selectedUsers.value.length} 个用户吗？`)) return;
    try {
