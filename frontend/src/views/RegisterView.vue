  ...
  <div class="max-w-md mx-auto mt-8 md:mt-16 mb-12">
    <div class="bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-gray-100">
      ...
      <form @submit.prevent="handleRegister" class="space-y-5">
        <div class="space-y-1">
          <label class="block text-sm font-semibold text-gray-700 ml-1">您的姓名</label>
          <div class="relative group">
            <User class="absolute left-4 top-3.5 h-5 w-5 text-gray-400 group-focus-within:text-primary transition-colors" />
            <input 
              v-model="name"
              type="text" 
              required
        ...
        <div class="space-y-1">
          <label class="block text-sm font-semibold text-gray-700 ml-1">设置密码</label>
          <div class="relative group">
        ...
        <div class="space-y-1">
          <label class="block text-sm font-semibold text-gray-700 ml-1">确认密码</label>
          <div class="relative group">
            <Lock class="absolute left-4 top-3.5 h-5 w-5 text-gray-400 group-focus-within:text-primary transition-colors" />
            <input 
        ...
        >
          ...
          <span v-else class="flex items-center">
            注册账号
            <UserPlus class="h-4 w-4 ml-2 group-hover:translate-x-1 transition-transform" />
          </span>
        </button>
      </form>
      ...
      <div class="mt-8 pt-6 border-t border-gray-100 text-center text-sm text-gray-500">
        已有账号？ 
        <RouterLink to="/login" class="text-primary font-bold hover:text-blue-700 hover:underline transition-all">直接登录</RouterLink>
      </div>
    </div>
  </div>
</template>
