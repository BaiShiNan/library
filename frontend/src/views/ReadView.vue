...
const handleNextPage = () => {
  const maxPage =
    fileType.value === "pdf" ? totalPdfPages.value : pages.value.length;
...
const readRemaining = async (reader, decoder) => {
  try {
    while (true) {
      const { done, value } = await reader.read();
      if (done) break;
...
<template>
  <div
    ...
    :class="
      isDarkMode ? 'bg-[#1a1a1a] text-gray-300' : 'bg-[#f8f9fa] text-gray-800'
    "
  >
    <!-- Top Bar -->
    <header
      ...
      :class="[
        isDarkMode
          ? 'bg-[#252525] border-gray-700 shadow-gray-900/50'
    ...
    >
      <div class="flex items-center gap-4">
        <button
          @click="router.back()"
          class="p-2 rounded-full hover:bg-opacity-10 hover:bg-gray-500 transition-colors"
        >
          <ArrowLeft class="h-5 w-5" />
        </button>
        <h1 class="font-bold text-lg truncate max-w-[200px] md:max-w-md">
          {{ book?.title || "加载中..." }}
        </h1>
      </div>
      ...
      <div v-if="fileType === 'txt'" class="flex items-center space-x-2">
        <div
          class="flex items-center bg-opacity-10 bg-gray-500 rounded-lg p-1 mr-2"
        >
          <button
            @click="decreaseFontSize"
            class="p-1.5 rounded hover:bg-opacity-20 hover:bg-gray-400 transition"
            title="减小字号"
          >
    ...
    >
      ...
      <div
        ...
        class="w-full h-full overflow-auto flex justify-center bg-gray-200 py-8"
      >
        <div
      ...
      >
        <div
          class="max-w-3xl mx-auto px-6 py-12 md:py-20 min-h-full flex flex-col"
        >
          <div
          ...
          <div
            class="mt-8 flex items-center justify-between text-sm opacity-50 select-none"
          >
    ...
    >
      ...
      <div class="text-sm font-medium opacity-80 font-sans">
        第 {{ currentPage }} 页 / 共
        {{ fileType === "pdf" ? totalPdfPages : pages.length || 1 }} 页
      </div>

      <button
        @click="handleNextPage"
        :disabled="
          currentPage >= (fileType === 'pdf' ? totalPdfPages : pages.length)
        "
        class="flex items-center gap-2 px-4 py-2 rounded-full hover:bg-opacity-10 hover:bg-gray-500 transition disabled:opacity-30 disabled:cursor-not-allowed font-medium"
      >
